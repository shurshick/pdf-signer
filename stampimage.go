...
func CreateStampImage(path string, d StampData) error {
    ...
    // Заголовок
    drawText(img, 20, 28, "ЭЛЕКТРОННАЯ ПОДПИСЬ", titleFace, blue)
    ...
    left1 := "Владелец: " + safeText(d.Owner)
    left2 := "Издатель: " + safeText(d.Issuer)
    left3 := "Дата: " + safeText(d.SignedAt)

    right1 := "Серийный номер: " + safeText(d.Serial)
    right2 := "SHA1: " + safeText(d.Thumbprint)
    right3 := "ЭП: " + safeText(d.SignatureFN)
