...
func main() {
    a := app.New()
    w := a.NewWindow("PDF signer + stamp")
    ...
    pdfLabel := widget.NewLabel("PDF не выбран")
    outLabel := widget.NewLabel("Выходной PDF не выбран")
    certInfoLabel := widget.NewLabel("Сертификат не выбран")
    ...
    selectPDFBtn := widget.NewButton("Выбрать PDF", func() {
    ...
    selectOutBtn := widget.NewButton("Сохранить PDF как", func() {
    ...
    runBtn := widget.NewButton("Подписать и поставить штамп", func() {
	if pdfPath == "" {
	    dialog.ShowInformation("Ошибка", "Выберите PDF", w)
	    return
	}
	if selectedCert.SubjectCN == "" && selectedCert.Serial == "" {
	    dialog.ShowInformation("Ошибка", "Выберите сертификат", w)
	    return
	}
	if outPDF == "" {
	    dialog.ShowInformation("Ошибка", "Укажите выходной PDF", w)
	    return
	}
	...
	widget.NewLabel("Сертификат:"),
	certSelect,
	certInfoLabel,
	widget.NewSeparator(),

	widget.NewForm(
	    widget.NewFormItem("Масштаб", scaleEntry),
	),
