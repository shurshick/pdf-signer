...
func GetCertificates() ([]CertInfo, error) {
    ...
    if err != nil {
	return nil, fmt.Errorf("ошибка certmgr: %v\n%s", err, string(out))
    }

    certs := parseCertmgrList(string(out))
    if len(certs) == 0 {
	return nil, fmt.Errorf("сертификаты в хранилище uMy не найдены")
    }
    return certs, nil
}
...
func parseCertmgrList(s string) []CertInfo {
	...
	switch {
	case strings.HasPrefix(line, "Субъект"):
	    cur.SubjectCN = extractCN(afterColon(line))
	case strings.HasPrefix(line, "Издатель"):
	    cur.IssuerCN = extractCN(afterColon(line))
	case strings.HasPrefix(line, "Серийный номер"):
	    cur.Serial = strings.TrimSpace(afterColon(line))
	case strings.HasPrefix(line, "SHA1 отпечаток"):
	    cur.Thumbprint = strings.TrimSpace(afterColon(line))
	case strings.HasPrefix(line, "Контейнер"):
	    cur.Container = strings.TrimSpace(afterColon(line))
	case strings.HasPrefix(line, "Имя провайдера"):
...
func buildCertLabel(c CertInfo) string {
    ...
    if c.IssuerCN != "" {
	parts = append(parts, "Издатель: "+c.IssuerCN)
    }
    if c.Serial != "" {
